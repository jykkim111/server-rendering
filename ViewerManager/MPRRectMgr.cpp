...
void MPRRectMgr::initSamplingCount(int w_cnt, int h_cnt, int c) {
...
void MPRRectMgr::setSamplingCount(int w_cnt, int h_cnt, int c, bool is_downsampling) {
	initSamplingCount(w_cnt, h_cnt, c);


	if (is_downsampling) {
		m_output_height = h_cnt;
		m_output_width = std::ceil(h_cnt * m_panel_wh_ratio);

	}
	else {
		m_output_height = m_panel_height;
		m_output_width = m_panel_width;
	}
	m_axial_w_sampling_len = m_axial_size * m_panel_wh_ratio / (w_cnt - 1);
	m_axial_h_sampling_len = m_axial_size / (h_cnt - 1);
	m_coronal_w_sampling_len = m_coronal_size * m_panel_wh_ratio / (w_cnt - 1);
	m_coronal_h_sampling_len = m_coronal_size / (h_cnt - 1);
	m_sagittal_w_sampling_len = m_sagittal_size * m_panel_wh_ratio / (w_cnt - 1);
...
/* get slice info */
std::tuple<int, int, float, float, cgip::CgipVector, cgip::CgipVector, cgip::CgipPoint> MPRRectMgr::getAxialInfo() {
	return {
...
std::tuple<int, int, float, float, cgip::CgipVector, cgip::CgipVector, cgip::CgipPoint> MPRRectMgr::getCoronalInfo() {
	return {
		m_w_sampling_cnt, m_h_sampling_cnt,
...
std::tuple<int, int, float, float, cgip::CgipVector, cgip::CgipVector, cgip::CgipPoint> MPRRectMgr::getSagittalInfo() {
	return {
		m_w_sampling_cnt, m_h_sampling_cnt,
...
SlicePacket MPRRectMgr::getSlicePacket(SliceType type) {
	SlicePacket packet;
	...
	packet.image_size = { m_output_width, m_output_height };
...
void MPRRectMgr::setRawMPR(SliceType slice_type) {
...
void MPRRectMgr::init_MPR(GLMgr* gl_mgr, VolumeMgr* vol_mgr)
{
	m_gl_mgr = gl_mgr;
	m_vol_mgr = vol_mgr;
...
void MPRRectMgr::init_position(int axial_loc)
{
	...
	m_axial_w_sampling_len = (m_axial_size * m_panel_wh_ratio) / (m_w_sampling_cnt - 1);
	m_axial_h_sampling_len = m_axial_size / (m_h_sampling_cnt - 1);
	m_coronal_w_sampling_len = (m_coronal_size * m_panel_wh_ratio) / (m_w_sampling_cnt - 1);
	m_coronal_h_sampling_len = m_coronal_size / (m_h_sampling_cnt - 1);
	m_sagittal_w_sampling_len = (m_sagittal_size * m_panel_wh_ratio) / (m_w_sampling_cnt - 1);
...
void MPRRectMgr::init_position(SliceType slice_type, int axial_loc)
{
	float width_mm = m_gl_mgr->getWidthMM();
	float height_mm = m_gl_mgr->getHeightMM();
	float depth_mm = m_gl_mgr->getDepthMM();
...
void MPRRectMgr::_resetTopLeftPt()
{
	...
	m_axial_topleft_pt = center_pt + m_axial_w_dir * (-(m_axial_size * m_panel_wh_ratio) / 2) + m_axial_h_dir * (-m_axial_size / 2);
	//m_coronal_topleft_pt = center_pt + (m_coronal_w_dir + m_coronal_h_dir) * (-m_coronal_size / 2);
	m_coronal_topleft_pt = center_pt + m_coronal_w_dir * (-(m_coronal_size * m_panel_wh_ratio) / 2) + m_coronal_h_dir * (-m_coronal_size / 2);
	//m_sagittal_topleft_pt = center_pt + (m_sagittal_w_dir + m_sagittal_h_dir) * (-m_sagittal_size / 2);
	m_sagittal_topleft_pt = center_pt + m_sagittal_w_dir * (-(m_sagittal_size * m_panel_wh_ratio) / 2) + m_sagittal_h_dir * (-m_sagittal_size / 2);



}

bool MPRRectMgr::_is_outside(VolumeMgr* vol_mgr, cgip::CgipPoint p)
{
...
int MPRRectMgr::move_slice(SliceType slice_type, VolumeMgr* vol_mgr, float x, float y, bool h, bool v)
{
	...
	switch (slice_type) {
	case AXIAL_SLICE:
		new_axis_center = m_axial_topleft_pt + (x * m_axial_w_dir * m_panel_wh_ratio + y * m_axial_h_dir) * m_axial_size;
		horizontal_n_dir = m_coronal_n_dir;
		vertical_n_dir = m_sagittal_n_dir;
		break;
	case CORONAL_SLICE:
		new_axis_center = m_coronal_topleft_pt + (x * m_coronal_w_dir * m_panel_wh_ratio + y * m_coronal_h_dir) * m_coronal_size;
		horizontal_n_dir = m_axial_n_dir;
		vertical_n_dir = m_sagittal_n_dir;
		break;
	case SAGITTAL_SLICE:
		new_axis_center = m_sagittal_topleft_pt + (x * m_sagittal_w_dir * m_panel_wh_ratio + y * m_sagittal_h_dir) * m_sagittal_size;
		horizontal_n_dir = m_axial_n_dir;
	...
	if (h)
		new_axis_center = new_axis_center + horizontal_d_vec * m_panel_wh_ratio;
	if (v)
	...
	case AXIAL_SLICE:
		if (h)
			m_coronal_topleft_pt = m_coronal_topleft_pt + horizontal_d_vec * m_panel_wh_ratio;;
		if (v)
			m_sagittal_topleft_pt = m_sagittal_topleft_pt + vertical_d_vec;
		break;
	case CORONAL_SLICE:
		if (h)
			m_axial_topleft_pt = m_axial_topleft_pt + horizontal_d_vec * m_panel_wh_ratio;;
		if (v)
			m_sagittal_topleft_pt = m_sagittal_topleft_pt + vertical_d_vec;
		break;
	case SAGITTAL_SLICE:
...
int MPRRectMgr::scroll_slice(SliceType slice_type, VolumeMgr* vol_mgr, float d)
{
...
int MPRRectMgr::align_center(SliceType slice_type, VolumeMgr* vol_mgr, float x, float y)
{
	cgip::CgipVector d_vec;
	cgip::CgipPoint new_axis_center;
	switch (slice_type) {
...
int MPRRectMgr::zoom_slice(SliceType slice_type, VolumeMgr* vol_mgr, float d)
{
	...
	case AXIAL_SLICE:
		...
		m_axial_topleft_pt = m_axial_topleft_pt - (d / 2) * (m_axial_w_dir * m_panel_wh_ratio + m_axial_h_dir);
		break;
...
int MPRRectMgr::zoom_slice_ratio(SliceType slice_type, VolumeMgr* vol_mgr, float d)
{
	float move_amount = 0;
...
int MPRRectMgr::slice_panning(SliceType slice_type, VolumeMgr* vol_mgr, float dx, float dy)
{
...
int MPRRectMgr::thickness(SliceType slice_type, int v) {
	switch (slice_type) {
	case AXIAL_SLICE:
		m_axial_thickness = v;
		break;
...
int MPRRectMgr::inverse(SliceType slice_type, bool v) {
	switch (slice_type) {
	case AXIAL_SLICE:
		m_axial_inverse = v;
		break;
...
float MPRRectMgr::_add_angle(float a, float b)
{
	float res = a + b;
...
int MPRRectMgr::rotate_slice(SliceType slice_type, float a)
{
...
int MPRRectMgr::windowing(SliceType slice_type, float dx, float dy) {
	/*
	m_window_width += dx;
	m_window_level -= dy;
...
std::tuple<float, float, float> MPRRectMgr::getSliceLineInfo(SliceType slice_type)
{
	...
	case AXIAL_SLICE:
		...
		axis_x = _get_projection(target, m_axial_w_dir) / (m_axial_size * m_panel_wh_ratio);
	...
	case CORONAL_SLICE:
		...
		axis_x = _get_projection(target, m_coronal_w_dir) / (m_coronal_size * m_panel_wh_ratio);
	...
	case SAGITTAL_SLICE:
		...
		axis_x = _get_projection(target, m_sagittal_w_dir) / (m_sagittal_size * m_panel_wh_ratio);
...
std::tuple<float, float> MPRRectMgr::getThicknessInfo(SliceType slice_type) {
...
cgip::CgipPlane MPRRectMgr::_setMPR(cgip::CgipPoint& topleft_p,
	...
	int bytesPerPixel = 3;
	int strideSize = (m_output_width * (4 - bytesPerPixel)) % 4;
	int bytesPerLine = m_output_width * 3 + strideSize;
	int size = bytesPerLine * m_output_height;
	GLubyte* imageData = new GLubyte[size];


	glFinish();
	//glReadPixels(0, 0, w_cnt, h_cnt, GL_RGB, GL_UNSIGNED_BYTE, imageData);
	glReadPixels(0, 0, m_output_width, m_output_height, GL_RGB, GL_UNSIGNED_BYTE, imageData);

	//cgip::CgipPlane new_plane(w_cnt, h_cnt, 3);
	cgip::CgipPlane new_plane(m_output_width, m_output_height, 3);
	...
	//	for (unsigned int j = 0; j < new_plane.getHeight(); ++j)
	//	{
	//		new_plane.setPixelValue(i, j, 0, imageData[3 * m_output_width * j + 3 * i + 0]);
	//		new_plane.setPixelValue(i, j, 1, imageData[3 * m_output_width * j + 3 * i + 1]);
	//		new_plane.setPixelValue(i, j, 2, imageData[3 * m_output_width * j + 3 * i + 2]);
	//	}
	//}
...
float MPRRectMgr::_get_projection(cgip::CgipVector a, cgip::CgipVector b)
{
	float result, pr;
	pr = a.dot(b);
	result = (pr * b).len();
...
float MPRRectMgr::_get_dist_next_slice(float d, float cur_loc, float spacing) {
	float temp_loc = 0.0;
	float next_loc = 0.0;
	cgip::CgipPoint new_axis_center;
...
cgip::CgipPoint MPRRectMgr::getCoordInfo(SliceType slice_type, float x, float y)
{
	cgip::CgipPoint pt;
	switch (slice_type) {
	case AXIAL_SLICE:
...
std::tuple<int, int, int> MPRRectMgr::getSliceNumInfo(SliceType slice_type)
{
	float x_num, y_num, z_num;
	switch (slice_type) {
	case AXIAL_SLICE:
...
std::tuple<int, int> MPRRectMgr::getPanningInfo(SliceType slice_type)
{
	float x_num, y_num;
	switch (slice_type) {
	case AXIAL_SLICE:
...
void MPRRectMgr::getViewingPlane(
...
std::tuple<cgip::CgipPoint, cgip::CgipPoint> MPRRectMgr::_find_intersection_point(cgip::CgipPoint center, cgip::CgipVector dir) {
