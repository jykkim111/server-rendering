...
uniform sampler1D rayStartingPoint;

uniform vec3 startingPoint;
uniform vec3 indicator_dir;
uniform vec3 n_dir;
...
void main()
{
	...
	if(!thickness){
		if(slice_type < 1){
			path_point = texelFetch(rayStartingPoint, int(vert_out.x), 0);
			cur = path_point.xyz + path_len * n_dir * vert_out.y;
		}
		else{
			path_point = texelFetch(rayStartingPoint, int(vert_out.y), 0);
			cur = path_point.xyz + path_len * n_dir * vert_out.x;
		}	
