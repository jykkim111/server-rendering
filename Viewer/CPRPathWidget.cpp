...
CPRPathWidget::CPRPathWidget(QWidget* parent, const char* name, SliceType slice_type) : QGraphicsView(parent) {
	// Set name
	...
	connect(plus_button, &QPushButton::clicked, this, &CPRPathWidget::slotZoomIn);
	connect(minus_button, &QPushButton::clicked, this, &CPRPathWidget::slotZoomOut);
...
void CPRPathWidget::setMode(int m) {
	m_mode = m;
}
void CPRPathWidget::toggleDrawGuideLine() {
...
void CPRPathWidget::setSlice(SlicePacket slice, cgip::CgipPlane plane) {
...
void CPRPathWidget::slotZoomOut() {
...
void CPRPathWidget::slotZoomRatioClicked() {
	m_zoom_ratio_box->setVisible(false);
	input_zoom_ratio->setVisible(true);
}

void CPRPathWidget::slotChangeZoomRatio() {
...
void CPRPathWidget::_setWindowing() {
...
/*
int CPRPathWidget::_applyWindowing(int v) {
...
QLabel* CPRPathWidget::_setInfoBox(int i, bool align_top) {
	// Coord Info
...
ClickableLabel* CPRPathWidget::_setZoomBox(int i, bool align_top) {
	// Coord Info
...
QLineEdit* CPRPathWidget::_setEditBox(int i, bool align_top) {
	// Coord Info
...
void CPRPathWidget::setCoordInfo(float x, float y) {
...
float CPRPathWidget::round_float(float var) {
	float value = (int)(var * 100 + .5);
	return (float)value / 100;
}


void CPRPathWidget::setZoomRatioInfo(float ratio) {
...
QLabel* CPRPathWidget::_setFPSBox(int i, bool align_top) {
	QLabel* info_box = new QLabel(this);
...
void CPRPathWidget::setFPSInfo(int fps) {
	QString msg;
...
void CPRPathWidget::setSliceInfo(float x, float y) {
...
ClickType CPRPathWidget::_get_clicktype(QMouseEvent* e)
{
...
float CPRPathWidget::_get_mouse_angle(QPointF p) {
...
void CPRPathWidget::setPanCenterPoint(const QPointF& newCenterPoint)
{
	m_pan_center_point = newCenterPoint;
	this->centerOn(m_pan_center_point);
}

void CPRPathWidget::mouseMoveEvent(QMouseEvent* event)
{
	if (is_panning) {
...
void CPRPathWidget::mousePressEvent(QMouseEvent* event) {
...
void CPRPathWidget::mouseReleaseEvent(QMouseEvent* event) {
...
void CPRPathWidget::mouseDoubleClickEvent(QMouseEvent* event) {
...
void CPRPathWidget::wheelEvent(QWheelEvent* event) {
	if (m_mode < 0)
		return;
...
void CPRPathWidget::leaveEvent(QEvent* event) {
